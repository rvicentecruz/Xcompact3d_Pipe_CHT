...
contains
  ...
  subroutine overall_statistic(ux1,uy1,uz1,phi1,pp3,ep1)
    ...
    !====Debug
    if (itime.lt.initstat.or.itype.eq.itype_pipe) then
       return
    endif
    ...
    !!$   call axial_averaging(pmean)
    !!$   call decomp_2d_write_plane(1,pmean,1,1,filename)
    !!$   write(filename,"('umean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,umean,filename,1)
    !!$   call axial_averaging(umean)
    !!$   call decomp_2d_write_plane(1,umean,1,1,filename)
    !!$   write(filename,"('vmean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,vmean,filename,1)
    !!$   call axial_averaging(vmean)
    !!$   call decomp_2d_write_plane(1,vmean,1,1,filename)
    !!$   write(filename,"('wmean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,wmean,filename,1)
    !!$   call axial_averaging(wmean)
    !!$   call decomp_2d_write_plane(1,wmean,1,1,filename)
    !!$   write(filename,"('uumean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,uumean,filename,1)
    !!$   call axial_averaging(uumean)
    !!$   call decomp_2d_write_plane(1,uumean,1,1,filename)
    !!$   write(filename,"('vvmean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,vvmean,filename,1)
    !!$   call axial_averaging(vvmean)
    !!$   call decomp_2d_write_plane(1,vvmean,1,1,filename)
    !!$   write(filename,"('wwmean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,wwmean,filename,1)
    !!$   call axial_averaging(wwmean)
    !!$   call decomp_2d_write_plane(1,wwmean,1,1,filename)

    !!$   write(filename,"('uvmean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,uvmean,filename,1)
    !!$   call axial_averaging(uvmean)
    !!$   call decomp_2d_write_plane(1,uvmean,1,1,filename)
    !!$   write(filename,"('uwmean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,uwmean,filename,1)
    !!$   call axial_averaging(uwmean)
    !!$   call decomp_2d_write_plane(1,uwmean,1,1,filename)
    !!$   write(filename,"('vwmean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,vwmean,filename,1)  
    !!$   call axial_averaging(vwmean)
    !!$   call decomp_2d_write_plane(1,vwmean,1,1,filename)

    !!$   write(filename,"('kmean.dat',I7.7)") itime
    !!$   !call decomp_2d_write_one(1,half*(uumean+vvmean+wwmean),filename,1)
    !!$   tmean(:,:,:)=half*(uumean(:,:,:)+vvmean(:,:,:)+wwmean(:,:,:))
    !!$   call axial_averaging(tmean)
    !!$   call decomp_2d_write_plane(1,tmean,1,1,filename)

    !!$   if (iscalar==1) then
    !!$      do is=1, numscalar
    !!$         write(filename,"('phi',I2.2,'mean.dat',I7.7)") is, itime
    !!$         !call decomp_2d_write_one(1,phimean(:,:,:,is),filename,1)
    !!$         call axial_averaging(phimean(:,:,:,is))
    !!$         call decomp_2d_write_plane(1,phimean(:,:,:,is),1,1,filename)
    !!$         write(filename,"('phiphi',I2.2,'mean.dat',I7.7)") is, itime
    !!$         !call decomp_2d_write_one(1,phiphimean(:,:,:,is),filename,1)
    !!$         call axial_averaging(phiphimean(:,:,:,is))
    !!$         call decomp_2d_write_plane(1,phiphimean(:,:,:,is),1,1,filename)
    !!$      enddo
    !!$   endif
